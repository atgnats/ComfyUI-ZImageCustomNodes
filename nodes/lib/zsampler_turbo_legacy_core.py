...
def zsampler_turbo_legacy_core(latent_input             : dict[str, Any],
    ...
    # if no sampler is explicitly requested, fall back to the tested default
    if primary_sampler is None:
        primary_sampler = "euler"
    if refiner_sampler is None:
        refiner_sampler = primary_sampler

    primary_sampler_object = comfy.samplers.sampler_object(primary_sampler)
    refiner_sampler_object = comfy.samplers.sampler_object(refiner_sampler)
    ...
    if noise_bias_scale != 0 and noise_bias_estimation != "none" and denoise >= 0.99:
        bias = calculate_denoise_bias(latent_input, model, seed, positive, positive,
                                      sampler     = primary_sampler_object,
    ...
    latent_output = execute_3_stage_denoising(latent_input,
                                              ...
                                              sampler1                = primary_sampler_object,
                                              sampler2                = primary_sampler_object,
                                              sampler3                = refiner_sampler_object,
                                              sigmas1                 = sigmas1,
                                              sigmas2                 = sigmas2,
                                              sigmas3                 = sigmas3,
                                              sigma_limit             = denoise,
...
def execute_3_stage_denoising(latent_image,
    ...
    Args:
        ...
        sampler1    : ComfyUI object representing the sampler used for the first stage of denoising.
        sampler2    : ComfyUI object representing the sampler used for the second stage of denoising.
        sampler3    : ComfyUI object representing the sampler used for the third stage of denoising.
